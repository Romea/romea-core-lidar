#include "romea_lidar/LIDAR3DFrameBuilderBase.hpp"
...
void LIDAR3DFrameBuilderBase<Scalar>::init(const LIDAR3D & lidar)
{
  ...
  numberOfElevationAngles_ = size_t(lidar.getElevationAperture()/lidar.getElevationAngleIncrement())+1;
  ...
  if(scanStorageOrder_== LIDAR3D::ScanStorageOrder::ELEVATION_MAJOR)
  {
    for(size_t i=0; i<numberOfAzimutAngles_ ; i++ )
    {
      ...
      {
        Scalar elevationAngle = lidar.getMininalElevationAngle() + j*lidar.getElevationAngleIncrement();
  ...
  else
  {
    ...
    {
      Scalar elevationAngle = lidar.getMininalElevationAngle() +i*lidar.getElevationAngleIncrement();
